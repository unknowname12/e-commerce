<x-layouts.main title="Create">
    <div class="min-h-screen w-full mx-auto">
        <div class="block p-6 bg-white border border-gray-200 rounded-lg bg-gray-300/20">
            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Create Product</h5>
            <hr class="my-[1rem]">
            <form method="post" action="{{ route('product.store')  }}" enctype="multipart/form-data">
                @csrf
                <div class="flex flex-col gap-3">
                    <div class="flex flex-col gap-1 justify-center items-center w-full">
                        <img title="Images" id="preview" class="border-[2px] h-[200px] w-[300px] object-cover rounded-lg" src="https://placehold.co/200x300" alt="Images" />
                    </div>
                    ...
                    <div class="flex flex-col gap-1 justify-start items-start w-full">
                        ...
                        <input class="w-full border-gray-300 bg-gray-200/60 rounded-lg" placeholder="Product Title" type="text" name="title" id="title" value="{{ old('title') }}">
                        <x-error :messages="$errors->get('title')"/>
                    </div>
                    <div class="flex flex-col gap-1 justify-start items-start w-full">
                        <label for="price">Price:</label>
                        <input class="w-full border-gray-300 bg-gray-200/60 rounded-lg" placeholder="Product Price" type="number" min="5000" name="price" id="price" value="{{ old('price') }}">
